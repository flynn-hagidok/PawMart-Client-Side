import React, { use, useState } from 'react';
import { AuthContext } from '../context/AuthContext/AuthContext';
import { Helmet } from 'react-helmet-async';
import { Link, useNavigate } from 'react-router';
import Loading from './Loading';

const Register = () => {

    const { signInWithGoogle, createUser, userProfile } = use(AuthContext)
    const [err, setErr] = useState("");
    const [regLoading, setRegLoading] = useState(false);

    const navigate = useNavigate();

    const handleGoogleLogin = () => {

        setRegLoading(true);

        signInWithGoogle()
            .then(result => {
                navigate("/", { replace: true })

            }).catch(err => {
                setErr(err.message);
            }).finally(() => {
                setRegLoading(false)
            })
    }

    const handleNewUser = (e) => {
        e.preventDefault();

        setRegLoading(true);

        const form = e.target;
        const email = form.email.value;
        const password = form.password.value;
        const photoURL = form.photo.value;
        const name = form.name.value;

        const newUser = { email, password, photoURL, name }
        const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z]).{6,}$/;

        setErr("");

        if (!passwordRegex.test(password)) {
            setRegLoading(false);
            return setErr("Password must be at least 6 characters long and contain at least one uppercase and one lowercase letter. ")
        }

        createUser(email, password)
            .then(async () => {
                await userProfile(name, photoURL);
                form.reset();
                navigate("/", { replace: true })
            }).catch(err => {
                const error = err.message;
                setErr(error);
            }).finally(() => {
                setRegLoading(false)
            })
    }

    return (
        <div className="min-h-screen flex items-center justify-center px-4 py-10">
            <Helmet>
                <title>PawMart | Register</title>
            </Helmet>
            <div className="w-full max-w-md shadow-2xl rounded-xl p-8">
                <h2 className="text-3xl font-bold text-center text-primary">
                    Register Now
                </h2>
                <p className="text-center text-gray-500 mt-2 mb-6">
                    Create your PawMart account
                </p>
                <form onSubmit={handleNewUser} className="space-y-2">
                    <div>
                        {
                            err ? <p className='p-2 px-4 text-red-500 bg-red-200 rounded-md'>{err}</p>
                                : ""
                        }
                    </div>
                    <div>
                        <label className="block mb-2 font-medium">
                            Full Name
                        </label>
                        <input
                            type="text" name="name" placeholder="Enter your name" className="input input-bordered w-full" required
                        />
                    </div>
                    <div>
                        <label className="block mb-2 font-medium">
                            Email
                        </label>
                        <input
                            type="email" name="email" placeholder="Enter your email" className="input input-bordered w-full" required
                        />
                    </div>
                    <div>
                        <label className="block mb-2 font-medium">
                            Photo URL
                        </label>
                        <input
                            type="text" name="photo" placeholder="Enter photo URL" className="input input-bordered w-full"
                        />
                    </div>
                    <div>
                        <label className="block mb-2 font-medium">
                            Password
                        </label>
                        <input
                            type="password" name="password" placeholder="Enter your password" className="input input-bordered w-full" required
                        />
                    </div>
                    <button
                        type="submit" disabled={regLoading}
                        className="btn bg-linear-to-r from-secondary to-base-200 w-full mt-2"
                    >
                        {
                            regLoading ? <Loading></Loading> : "Register"
                        }
                    </button>
                    <p className='text-center font-semibold'>Or</p>
                    <button type="button" disabled={regLoading} onClick={handleGoogleLogin} className="btn bg-white text-black border-[#e5e5e5] w-full">
                        <svg aria-label="Google logo" width="16" height="16" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><g><path d="m0 0H512V512H0" fill="#fff"></path><path fill="#34a853" d="M153 292c30 82 118 95 171 60h62v48A192 192 0 0190 341"></path><path fill="#4285f4" d="m386 400a140 175 0 0053-179H260v74h102q-7 37-38 57"></path><path fill="#fbbc02" d="m90 341a208 200 0 010-171l63 49q-12 37 0 73"></path><path fill="#ea4335" d="m153 219c22-69 116-109 179-50l55-54c-78-75-230-72-297 55"></path></g></svg>
                        Login with Google
                    </button>
                    <p className="text-center text-sm">
                        Already have an account?{" "}
                        <Link to="/login" className="text-primary font-semibold cursor-pointer hover:underline">
                            Login
                        </Link>
                    </p>
                </form>
            </div>
        </div>
    );
};

export default Register;